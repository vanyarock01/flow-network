...
function makeEditor(){
  ...
  function solveFlow (){
    console.debug("networkGraph:", networkGraph.links());
  ...
  function textboxHandler() {
    ...
    // Nothing selected
      ...
      alert("Пожалуйста, выберите ребро или вершину.");
    }

    // Changing Node Name
    if (selected_node) {
      if (contents.length > 2) {
        alert("Возможная длина <= 2");
      }
      else {
  ...
  var tmpNodes = [
      {id: 0, name: '0', fixed: true, x: (percentile), y: height/2},
      {id: 1, name: '1', fixed: true, x: (width-percentile), y: height/2},
      
    ],
    tmpLinks = [
 /*     {source: tmpNodes[0], target: tmpNodes[2], capacity: 15},
      {source: tmpNodes[0], target: tmpNodes[3], capacity: 18 },
      ...
      {source: tmpNodes[4], target: tmpNodes[1], capacity: 7 },
      {source: tmpNodes[5], target: tmpNodes[4], capacity: 6 },
      {source: tmpNodes[5], target: tmpNodes[1], capacity: 12 },
*/
    ];
  ...
  function restart() {
    ...
    var g = path.enter().append('g')
      ...
      .on('mousedown', function(d) {
        ...
        textBox.attr('placeholder', "Новая пропускная способность");
        restart();
      });
    ...
    gC.on('mouseover', function(d) {
      ...
      .on('mousedown', function(d) {
        ...
        textBox.attr('placeholder', "Новое имя вершины");
